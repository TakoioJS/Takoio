/**
 * SQLite schema — 薄封装，表定义已迁入 ./schema-shared.ts 的 defineSchema 工厂。
 *
 * 外部用法保持不变：
 * - `import { comments } from './schema'`（命名导入）
 * - `import * as schema from './schema'`（namespace，用于 drizzle(client, { schema })）
 */

import { defineSchema } from './schema-shared'

const _schema = defineSchema('sqlite')
